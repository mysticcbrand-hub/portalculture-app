...
// ═══════════════════════════════════════════════════════════════════
// PORTAL COMPONENT - Minimalist entrance with breathing logo
// ═══════════════════════════════════════════════════════════════════
...
function GrainLayer() {
  return (
    <svg aria-hidden="true" style={{ position: 'fixed', inset: 0, width: '100%', height: '100%', opacity: 0.028, pointerEvents: 'none', zIndex: 0 }}>
      <filter id="auth-grain"><feTurbulence type="fractalNoise" baseFrequency="0.70" numOctaves="4" stitchTiles="stitch" /><feColorMatrix type="matrix" values="0.33 0.33 0.33 0 0 0.33 0.33 0.33 0 0 0.33 0.33 0.33 0 0 0 0 1 0" /></filter>
...
function BreathingLogo({ isMobile }: { isMobile: boolean }) {
  ...
  return (
    <motion.div style={{ position: 'relative', zIndex: 1, marginBottom: 64 }}>
      <motion.div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 80, height: 80, borderRadius: '50%', background: 'rgba(255, 255, 255, 0.04)', filter: 'blur(20px)', pointerEvents: 'none' }}
        animate={{ scale: [1, 1.4, 1], opacity: [0.4, 0.7, 0.4] }}
        transition={{ duration: 4.0, repeat: Infinity, ease: 'easeInOut', repeatType: 'mirror' }}
      />
      <motion.img src="/este_logo.ico" alt="Portal Culture" style={{ width: size, height: size, imageRendering: 'crisp-edges' as React.CSSProperties['imageRendering'], display: 'block', position: 'relative', zIndex: 1, filter: 'brightness(0.9) saturate(0)' }}
        animate={{ scale: [1, 1.018, 1], opacity: [0.82, 0.96, 0.82] }}
        transition={{ duration: 4.0, repeat: Infinity, ease: 'easeInOut', repeatType: 'mirror' }}
      />
    </motion.div>
  )
...
function TextLine({ line, isVisible }: { line: TextLineConfig; isVisible: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '0 0.28em', marginBottom: line.marginBottom ?? 0 }}>
      {words.map((word, i) => (
        <motion.span key={i} initial={{ opacity: 0, y: 10, filter: 'blur(8px)' }} animate={isVisible ? { opacity: 1, y: 0, filter: 'blur(0px)' } : { opacity: 0, y: 10, filter: 'blur(8px)' }} transition={{ duration: 0.65, delay: i * 0.075, ease: [0.25, 0.46, 0.45, 0.94] }}
          style={{ display: 'inline-block', fontSize: line.size, fontWeight: line.weight, color: line.color, letterSpacing: line.letterSpacing, textTransform: line.textTransform ?? 'none', lineHeight: 1.3, WebkitFontSmoothing: 'antialiased' }}
        >{word}</motion.span>
      ))}
    </div>
  )
...
function PortalEntrance({ onEnter }: { onEnter: () => void }) {
  ...
  useEffect(() => {
    ...
    TEXT_SEQUENCE.forEach((line) => {
      ...
      timers.push(setTimeout(() => setVisibleLines((prev) => new Set([...prev, line.id])), cumulativeDelay))

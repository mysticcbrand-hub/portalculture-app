    ...
    const handleMagicLink = async () => {
      try {
        ...
        if (!accessToken || !refreshToken || type !== 'magiclink') {
          setStatus('error')
          setMessage('El enlace de acceso es inválido o expiró.')
          return
        }
        ...
        if (error) {
          setStatus('error')
          setMessage('No pudimos validar tu acceso. Solicita un nuevo enlace.')
          return
        }

        setStatus('success')
        setMessage('Acceso confirmado. Redirigiendo al dashboard...')
        setTimeout(() => router.push(next), 800)
      } catch {
        setStatus('error')
        setMessage('Ocurrió un error al validar tu acceso.')
      }
    }
    ...
    <main className="min-h-screen flex items-center justify-center px-6 py-16 bg-black relative overflow-hidden">
      <div className="absolute inset-0">
        <div
          ...
          style={{
            background: `
              radial-gradient(ellipse 70% 50% at 50% 20%, rgba(168, 85, 247, 0.08) 0%, rgba(147, 51, 234, 0.05) 20%, rgba(126, 34, 206, 0.03) 40%, rgba(107, 33, 168, 0.018) 60%, transparent 80%),
              radial-gradient(ellipse 80% 60% at 50% 80%, rgba(59, 130, 246, 0.06) 0%, rgba(37, 99, 235, 0.04) 25%, rgba(29, 78, 216, 0.02) 50%, transparent 80%)
            `,
          }}
        />
        <div
